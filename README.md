<div align="center">

<img src="./logo.svg" width="160px" />

# Quick Lofi Landing Page

Landing page for **Quick Lofi**,  
a GNOME Shell extension to listen to lofi and ambient sounds.

One click. No clutter. Just focus.

[![GitHub stars](https://img.shields.io/github/stars/EuCaue/quick-lofi?style=for-the-badge&logo=github)](https://github.com/EuCaue/quick-lofi)
[![GNOME Extension](https://img.shields.io/badge/GNOME-Extension-blue?style=for-the-badge&logo=gnome)](https://extensions.gnome.org/extension/6904/quick-lofi/)
[![Ko-fi](https://img.shields.io/badge/Ko--fi-Support-ff5f5f?logo=kofi&logoColor=white&style=for-the-badge)](https://ko-fi.com/eucaue)

![GNOME Supported Version](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fgithub.com%2FEuCaue%2Fgnome-shell-extension-quick-lofi%2Fraw%2Fmaster%2Fsrc%2Fmetadata.json&query=%24%5B'shell-version'%5D&style=for-the-badge&logo=gnome&label=Compatible%20with%20GNOME)

</div>

## About

This page showcases **Quick Lofi**, a GNOME Shell extension that lets you play lofi and ambient sounds from the top bar.

No browser tabs.  
No heavy players.  
Just sound and focus.

## What Quick Lofi offers

• Minimal and distraction free experience  
• Native GNOME integration  
• Instant access from the top bar  
• Local files or online radios  
• Fully open source

## Features showcased

• One click play and pause  
• Volume control in the menu  
• Keyboard shortcuts  
• Drag and drop stations  
• Adjustable pop up size

## Open source

The extension source code, issues and feature requests live here:

<div align="center">

[![GitHub Repo](https://img.shields.io/badge/QuickLofi-Repository-181717?logo=github&logoColor=white&style=for-the-badge)](https://github.com/EuCaue/quick-lofi)

</div>

## Support

If Quick Lofi helps you focus, consider supporting the project:

<div align="center">

[![Ko-fi](https://img.shields.io/badge/Ko--fi-Support-ff5f5f?logo=kofi&logoColor=white&style=for-the-badge)](https://ko-fi.com/eucaue)

</div>

<small>
  <div align="center">
    Made with ❤️ by <a href="https://github.com/EuCaue">EuCaue</a>
  </div>
</small>
